...
try {
    ...
    // Create fallback objects to prevent script errors
    account = {
        createEmailPasswordSession: () => Promise.reject(new Error('Appwrite not configured')),
        create: () => Promise.reject(new Error('Appwrite not configured')),
        get: () => Promise.reject(new Error('Appwrite not configured')),
        deleteSession: () => Promise.reject(new Error('Appwrite not configured'))
    };
    
    databases = {
        createDocument: () => Promise.reject(new Error('Appwrite not configured')),
        updateDocument: () => Promise.reject(new Error('Appwrite not configured')),
        listDocuments: () => Promise.reject(new Error('Appwrite not configured')),
        getDocument: () => Promise.reject(new Error('Appwrite not configured'))
    };
    
    // Set fallback IDs
    DATABASE_ID = 'not-configured';
    LEAGUES_COLLECTION_ID = 'not-configured';
    LEAGUE_MEMBERS_COLLECTION_ID = 'not-configured';
    
    // Don't throw here - let the app continue running in offline mode
    console.warn('⚠️  Running in offline mode - multiplayer features will be disabled');
}
